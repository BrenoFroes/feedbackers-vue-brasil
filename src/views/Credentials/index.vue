<template>
  ...
  <div class="flex flex-col justify-center items-center h-64 bg-brand-gray">
    ...
    <p class="text-lg text-center text-gray-800 fontregular">
      Guia de instalação e geração de credenciais
    </p>
  </div>
  <div class="flex justify-center w-full h-full">
    <div class="flex flex-col items-start w-4/5 max-w-6xl py-10">
      <h1 class="text-3xl font-black text-brand-darkgray">
        Instalação e configuração</h1>
      ...
      <div v-else class="flex py-3 pl-5 mt-2 rounded justify-between items-center bg-brand-gray w-full lg:w-1/2">
        ...
        <div v-if="!state.hasError" class="flex items-center align-items-center justify-center ml-20 mr-5">
      ...
      <div v-else class="py-3 pl-5 pr-20 mt-2 rounded bg-brand-gray w-full lg:w-2/3 overflow-x-scroll">
        ...
        <pre v-else>&lt;script src="https://brenofroes-feedbacker-widget.netlify.app?api_key${{store.User.currentUser.apiKey}}"&gt;&lt;/script&gt;</pre>
      </div>
    </div>
  </div>
</template>
